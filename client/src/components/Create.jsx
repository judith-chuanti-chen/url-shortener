...
const Create = () =>{
    ...
    const [result, setResult] = useState({isSuccess: false, msg: errMsg, data:{}});
    ...
    const createUrl = () => {
        ...
        Axios.post('/api/url/create', {
            ...
            switch(response.status){
                case 200:
                    isSuccess = true;
                    msg = 'A short URL is successfully generated for this website!';
                    // data = response.data;
            ...
            if(err.response){
                switch(err.response.status){
                    ...
                    case 409:
                        isSuccess = false;
                        msg = 'Sorry, this short URL address has been taken! Please try again!';
                        break;
                    default:
                        isSuccess = false;
                        msg = errMsg;
                    ...
                    <Form.Group controlId="formCustomString">
                        ...
                        <Collapse in={custom}>
                            <div id="input-custom-string">
                                <Form.Label>Enter your custom label:</Form.Label>
                                <InputGroup className='mb-3'>
                                    <InputGroup.Prepend>
                                        <InputGroup.Text id="base-url">{window.location.origin + '/'}</InputGroup.Text>
                                    </InputGroup.Prepend>
                                    <FormControl  
                                        type="text" 
                                        aria-describedby="base-url" 
                                        onChange={(e) => {setCustomString(e.target.value); console.log(e.target.value);}}
                                        required />
                                </InputGroup>
                            </div>
                        </Collapse>
                        <Row className="mt-4">
                            <Button className="m-auto btn-lg" variant="primary" onClick={() => createUrl()}>Generate!</Button>
                        </Row>
                    </Form.Group>
                </Form>
            </Container>
